...
class AnnotationDriver
{
    ...
    public function __construct($cachePath, $debug = false)
    {
        ...
        $this->debug = $debug;
        AnnotationRegistry::registerFile(__DIR__ . '/../Annotations/AllAnnotations.php');
    }
    /**
    ...
    {
        $reader = new FileCacheReader(new AnnotationReader(), $this->cachePath, $this->debug);
        ...
        foreach ($reflClass->getProperties() as $property) {
            ...
            if ($propertyAnnotation) {
                ...
                $attributeList[] = new Attribute($propertyAnnotation->name, $property->getName(), $propertyAnnotation->type, (bool) $isId);
            } else {
                // manage relations
                ...
                if ($relation) {
                    ...
                    if (false === mb_strpos($targetEntity, '/')) {
                        $targetEntity = mb_substr($classname, 0, mb_strrpos($classname, '\\') + 1) . $targetEntity;
                    }
                    $relationList[] = new Relation($relation->name, $relation->type, $targetEntity);
                }
            }
        }
        $classMetadata = new ClassMetadata($classAnnotation->key, $classname, $classAnnotation->repository);
        $classMetadata->setAttributeList($attributeList);
        $classMetadata->setRelationList($relationList);
        return $classMetadata;
    }
    private function getPropertyAnnotation(Reader $reader, \ReflectionProperty $property, $classname)
    {
        return $reader->getPropertyAnnotation($property, 'Mapado\\RestClientSdk\\Mapping\\Annotations\\' . $classname);
    }
}