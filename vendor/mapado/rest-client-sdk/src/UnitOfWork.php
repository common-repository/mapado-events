...
class UnitOfWork
{
     ...
     */
    public function registerClean($id, object $entity)
    {
    ...
    private function getDirtyFields(array $newSerializedModel, array $oldSerializedModel, ClassMetadata $classMetadata)
    {
        ...
        foreach ($newSerializedModel as $key => $value) {
            ...
            if (!$currentRelation) {
                if (is_array($value) && !ArrayHelper::arraySame($value, $oldValue ?: []) || $value !== $oldValue) {
                    $dirtyFields[$key] = $value;
                }
                continue;
            }
            ...
            if (count(isset($value) ? $value : []) !== count(isset($oldValue) ? $oldValue : [])) {
     ...
     */
    private function addIdentifiers($newSerializedModel, array $dirtyFields, $idSerializedKey = null)
    {
        foreach ($newSerializedModel as $key => $value) {
    ...
    private static function getEntityId($stringOrEntity, $idSerializedKey)
    {
        ...
        return isset($stringOrEntity[$idSerializedKey]) ? $stringOrEntity[$idSerializedKey] : null;
    }
}