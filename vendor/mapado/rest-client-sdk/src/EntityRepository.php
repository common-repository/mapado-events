...
class EntityRepository
{
     ...
     */
    public function find($id, array $queryParams = [])
    {
        $hydrator = $this->sdk->getModelHydrator();
        ...
        if (null !== $entity) {
            ...
            // another register clean will be made in the Serializer if the id different from the called uri
        }
        return $entity;
    }
    ...
    public function update(object $model, array $serializationContext = [], array $queryParams = [])
    {
        $identifier = $model->{$this->getClassMetadata()->getIdGetter()}();
    ...
    public function persist(object $model, array $serializationContext = [], array $queryParams = [])
    {
        $mapping = $this->sdk->getMapping();
        $prefix = $mapping->getIdPrefix();
        $key = $mapping->getKeyFromModel($this->entityName);
        ...
        $newSerializedModel = $this->sdk->getSerializer()->serialize($model, $this->entityName, $serializationContext);
        $diff = $this->unitOfWork->getDirtyData($newSerializedModel, $oldSerializedModel, $this->getClassMetadata());
        $data = $this->restClient->post($this->addQueryParameter($path, $queryParams), $diff);
        $data = $this->assertNotObject($data, __METHOD__);
        if (null === $data) {
    ...
    protected function saveToCache($key, object $value = null)
    {
        $key = $this->normalizeCacheKey($key);
        $cacheItemPool = $this->sdk->getCacheItemPool();
        if ($cacheItemPool) {
    ...
    protected function addQueryParameter($path, array $params = [])
    {
        if (empty($params)) {
            return $path;
        }
    ...
    private function convertQueryParameters(array $queryParameters)
    {
        $mapping = $this->sdk->getMapping();
        return array_map(function ($item) use($mapping) {
            if (is_object($item)) {
                ...
                if ($mapping->hasClassMetadata($classname)) {
                    $idGetter = $mapping->getClassMetadata($classname)->getIdGetter();
    ...
    private function getClassMetadata()
    {
        if (!isset($this->classMetadata)) {
            $this->classMetadataCache = $this->sdk->getMapping()->getClassMetadata($this->entityName);
        }
     ...
     */
    private function assertArray($data, $methodName)
    {
        if (is_array($data)) {
            return $data;
        }
     ...
     * @return array|null
     */
    private function assertNotObject($data, $methodName)
    {
        if (!is_object($data)) {
            return $data;
        }
