...
class Serializer
{
     ...
     */
    public function serialize(object $entity, $modelName, array $context = [])
    {
        $out = $this->recursiveSerialize($entity, $modelName, 0, $context);
        if (is_string($out)) {
            throw new \RuntimeException('recursiveSerialize should return an array for level 0 of serialization. This should not happen.');
        }
        return $out;
    }
    public function deserialize(array $data, $className)
    {
            ...
            foreach ($attributeList as $attribute) {
                ...
                if ($relation) {
                    if (is_string($value)) {
                        ...
                        $relationClassMetadata = $this->mapping->getClassMetadata($targetEntity);
                        if ($relation->isManyToOne()) {
                            $value = $this->deserialize($value, $relationClassMetadata->getModelName());
                        } else {
                            ...
                            foreach ($value as $item) {
                                if (is_string($item)) {
                                    ...
                                    $list[] = $this->deserialize($item, $relationClassMetadata->getModelName());
                                }
                            }
                            $value = $list;
                        }
                ...
                if (isset($value)) {
                    if ('datetime' === $attribute->getType()) {
                        $this->setDateTimeValue($instance, $attributeName, $value);
                    } else {
                        $this->propertyAccessor->setValue($instance, $attributeName, $value);
                    }
                }
            }
        }
        ...
        if ($classMetadata->hasIdentifierAttribute()) {
            ...
            if ($idGetter) {
                ...
                $identifier = is_callable($callable) ? call_user_func($callable) : null;
                if ($identifier) {
                    $this->unitOfWork->registerClean((string) $identifier, $instance);
                }
            }
        }
        return $instance;
     ...
     * @return array|string
     */
    private function recursiveSerialize(object $entity, $modelName, $level = 0, array $context = [])
    {
            ...
            foreach ($attributeList as $attribute) {
                ...
                if (null === $data && $relation && $relation->isManyToOne() && $level > 0) {
                    ...
                    */
                    ...
                    $data = $phoneNumberUtil->format($data, PhoneNumberFormat::INTERNATIONAL);
                } elseif (is_object($data) && $relation && $this->mapping->hasClassMetadata($relation->getTargetEntity())) {
                    $relationClassMetadata = $this->mapping->getClassMetadata($relation->getTargetEntity());
                    if (!$relationClassMetadata->hasIdentifierAttribute()) {
                        $data = $this->recursiveSerialize($data, $relation->getTargetEntity(), $level + 1, $context);
                    } else {
                        $idAttribute = $relationClassMetadata->getIdentifierAttribute();
                        $idGetter = 'get' . ucfirst($idAttribute->getAttributeName());
                    ...
                    foreach ($data as $key => $item) {
                        if ($item instanceof DateTimeInterface) {
                            ...
                            $serializeRelation = !empty($context['serializeRelations']) && in_array($relation->getSerializedKey(), $context['serializeRelations']);
                            $newData[$key] = $this->recursiveSerialize($item, $relation->getTargetEntity(), $level + 1, ['serializeRelation' => $serializeRelation]);
                        } else {
                            $newData[$key] = $item;
                        }
                    }
    ...
    private function getClassMetadata(object $entity)
    {
        return $this->mapping->getClassMetadata(get_class($entity));
    }
    private function throwIfAttributeIsNotWritable(object $instance, $attribute)
    {
        if (!$this->propertyAccessor->isWritable($instance, $attribute)) {
            throw new MissingSetterException(sprintf('Property %s is not writable for class %s. Please make it writable. You can check the property-access documentation here : https://symfony.com/doc/current/components/property_access.html#writing-to-objects', $attribute, get_class($instance)));
        }
    }
    private function setDateTimeValue(object $instance, $attributeName, $value)
    {
        try {
            $this->propertyAccessor->setValue($instance, $attributeName, new DateTime($value));
        } catch (InvalidArgumentException $e) {
            if (false === mb_strpos($e->getMessage(), 'Expected argument of type "DateTimeImmutable", "DateTime" given')) {
                // not an issue with DateTimeImmutable, then rethrow exception
                throw $e;
            }
            // The excepted value is a DateTimeImmutable, so let's do that
            $this->propertyAccessor->setValue($instance, $attributeName, new DateTimeImmutable($value));
        } catch (\TypeError $e) {
            // this `catch` block can be dropped when minimum support of symfony/property-access is 3.4
            if (false === mb_strpos($e->getMessage(), 'must be an instance of DateTimeImmutable, instance of DateTime given')) {
                // not an issue with DateTimeImmutable, then rethrow exception
                throw $e;
            }
            // The excepted value is a DateTimeImmutable, so let's do that
