...
class FileReflection implements ReflectionInterface
{
    ...
    public function __construct($filename, $includeIfNotAlreadyIncluded = false)
    {
        ...
        if (!$fileRealPath) {
            throw new Exception\InvalidArgumentException(sprintf('No file for %s was found.', $filename));
        }
        if (!in_array($fileRealPath, get_included_files())) {
            if (!$includeIfNotAlreadyIncluded) {
                throw new Exception\RuntimeException(sprintf('File %s must be required before it can be reflected', $filename));
            }
    ...
    public function getClass($name = null)
    {
        ...
        throw new Exception\InvalidArgumentException(sprintf('Class by name %s not found.', $name));
    }
    /**
