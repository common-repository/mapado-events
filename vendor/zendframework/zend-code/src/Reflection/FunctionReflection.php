...
class FunctionReflection extends ReflectionFunction implements ReflectionInterface
{
    ...
    public function getDocBlock()
    {
        if ('' == ($comment = $this->getDocComment())) {
            throw new Exception\InvalidArgumentException(sprintf('%s does not have a DocBlock', $this->getName()));
        }
    ...
    public function getContents($includeDocBlock = true)
    {
        ...
        $lines = array_slice(file($fileName, FILE_IGNORE_NEW_LINES), $startLine - 1, $endLine - ($startLine - 1), true);
    ...
    public function getPrototype($format = FunctionReflection::PROTOTYPE_AS_ARRAY)
    {
        ...
        $prototype = ['namespace' => $this->getNamespaceName(), 'name' => substr($this->getName(), strlen($this->getNamespaceName()) + 1), 'return' => $returnType, 'arguments' => []];
