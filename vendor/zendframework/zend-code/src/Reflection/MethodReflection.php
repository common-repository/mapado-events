...
class MethodReflection extends PhpReflectionMethod implements ReflectionInterface
{
    ...
    public function getPrototype($format = MethodReflection::PROTOTYPE_AS_ARRAY)
    {
        ...
        $prototype = ['namespace' => $declaringClass->getNamespaceName(), 'class' => substr($declaringClass->getName(), strlen($declaringClass->getNamespaceName()) + 1), 'name' => $this->getName(), 'visibility' => $this->isPublic() ? 'public' : ($this->isPrivate() ? 'private' : 'protected'), 'return' => $returnType, 'arguments' => []];
        $parameters = $this->getParameters();
        foreach ($parameters as $parameter) {
            $prototype['arguments'][$parameter->getName()] = ['type' => $parameter->detectType(), 'required' => !$parameter->isOptional(), 'by_ref' => $parameter->isPassedByReference(), 'default' => $parameter->isDefaultValueAvailable() ? $parameter->getDefaultValue() : null];
        }
    ...
    public function getParameters()
    {
        ...
        while ($phpReflections && ($phpReflection = array_shift($phpReflections))) {
            $instance = new ParameterReflection([$this->getDeclaringClass()->getName(), $this->getName()], $phpReflection->getName());
    ...
    protected function extractMethodContents($bodyOnly = false)
    {
        ...
        $lines = array_slice(file($fileName, FILE_IGNORE_NEW_LINES), $this->getStartLine() - 1, $this->getEndLine() - ($this->getStartLine() - 1), true);
