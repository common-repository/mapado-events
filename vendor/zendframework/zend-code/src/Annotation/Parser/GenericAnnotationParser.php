...
class GenericAnnotationParser implements ParserInterface
{
    ...
    public function registerAnnotation($annotation)
    {
        ...
        if (!$annotation instanceof AnnotationInterface) {
            throw new Exception\InvalidArgumentException(sprintf('%s: expects an instance of %s\\AnnotationInterface; received "%s"', __METHOD__, __NAMESPACE__, is_object($annotation) ? get_class($annotation) : gettype($annotation)));
        }
        $class = $class ?: get_class($annotation);
        if (in_array($class, $this->annotationNames)) {
            throw new Exception\InvalidArgumentException(sprintf('An annotation for this class %s already exists', $class));
        }
    ...
    public function registerAnnotations($annotations)
    {
        if (!is_array($annotations) && !$annotations instanceof Traversable) {
            throw new Exception\InvalidArgumentException(sprintf('%s: expects an array or Traversable; received "%s"', __METHOD__, is_object($annotations) ? get_class($annotations) : gettype($annotations)));
        }
    ...
    public function setAlias($alias, $class)
    {
        if (!in_array($class, $this->annotationNames) && !$this->hasAlias($class)) {
            throw new Exception\InvalidArgumentException(sprintf('%s: Cannot alias "%s" to "%s", as class "%s" is not currently a registered annotation or alias', __METHOD__, $alias, $class, $class));
        }
