    ...
    public function setDeclares(array $declares)
    {
        foreach ($declares as $declare) {
            if (!$declare instanceof DeclareStatement) {
                throw new InvalidArgumentException(sprintf('%s is expecting an array of %s objects', __METHOD__, DeclareStatement::class));
            }
    ...
    public function generate()
    {
        ...
        if ($namespace) {
            ...
            if (preg_match('#/\\* Zend_Code_Generator_FileGenerator-NamespaceMarker \\*/#m', $output)) {
                $output = preg_replace('#/\\* Zend_Code_Generator_FileGenerator-NamespaceMarker \\*/#m', $namespace, $output, 1);
            } else {
                $output .= $namespace;
            }
        }
        ...
        if ($this->declares) {
            ...
            if (preg_match('#/\\* Zend_Code_Generator_FileGenerator-DeclaresMarker \\*/#m', $output)) {
                $output = preg_replace('#/\\* Zend_Code_Generator_FileGenerator-DeclaresMarker \\*/#m', $declareStatements, $output, 1);
            } else {
        ...
        if (!empty($uses)) {
            ...
            if (preg_match('#/\\* Zend_Code_Generator_FileGenerator-UseMarker \\*/#m', $output)) {
                $output = preg_replace('#/\\* Zend_Code_Generator_FileGenerator-UseMarker \\*/#m', $useOutput, $output, 1);
            } else {
                $output .= $useOutput;
            }
        }
