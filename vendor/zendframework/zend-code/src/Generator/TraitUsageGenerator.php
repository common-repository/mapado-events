...
class TraitUsageGenerator extends AbstractGenerator implements TraitUsageInterface
{
     ...
     * @return string|null
     */
    public function getUseAlias($use)
    {
        foreach ($this->uses as $key => $value) {
     ...
     * @return bool
     */
    public function isUseAlias($alias)
    {
        foreach ($this->uses as $key => $value) {
            $parts = explode(' as ', $key);
    ...
    public function addTraitAlias($method, $alias, $visibility = null)
    {
        ...
        $this->traitAliases[$traitAndMethod] = ['alias' => $alias, 'visibility' => $visibility];
        return $this;
    }
    /**
     * @inheritDoc
    ...
    public function removeTraitOverride($method, $overridesToRemove = null)
    {
        ...
        $overridesToRemove = !is_array($overridesToRemove) ? [$overridesToRemove] : $overridesToRemove;
