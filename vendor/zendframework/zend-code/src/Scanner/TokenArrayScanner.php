...
class TokenArrayScanner implements ScannerInterface
{
    ...
    protected function scan()
    {
        ...
        switch ($tokenType) {
            ...
            case T_NAMESPACE:
                $infos[$infoIndex] = ['type' => 'namespace', 'tokenStart' => $MACRO_TOKEN_LOGICAL_START_INDEX(), 'tokenEnd' => null, 'lineStart' => $token[2], 'lineEnd' => null, 'namespace' => null];
            ...
            // goto no break needed
            case T_USE:
                $infos[$infoIndex] = ['type' => 'use', 'tokenStart' => $MACRO_TOKEN_LOGICAL_START_INDEX(), 'tokenEnd' => null, 'lineStart' => $tokens[$tokenIndex][2], 'lineEnd' => null, 'namespace' => $namespace, 'statements' => [0 => ['use' => null, 'as' => null]]];
                $useStatementIndex = 0;
                $useAsContext = false;
                // start processing with next token
                if ($MACRO_TOKEN_ADVANCE() === false) {
            ...
            case T_REQUIRE_ONCE:
                ...
                static $includeTypes = [T_INCLUDE => 'include', T_INCLUDE_ONCE => 'include_once', T_REQUIRE => 'require', T_REQUIRE_ONCE => 'require_once'];
                $infos[$infoIndex] = ['type' => 'include', 'tokenStart' => $MACRO_TOKEN_LOGICAL_START_INDEX(), 'tokenEnd' => null, 'lineStart' => $tokens[$tokenIndex][2], 'lineEnd' => null, 'includeType' => $includeTypes[$tokens[$tokenIndex][0]], 'path' => ''];
                // start processing with next token
                if ($MACRO_TOKEN_ADVANCE() === false) {
                    goto SCANNER_END;
                }
            ...
            case T_INTERFACE:
            case T_TRAIT:
                $infos[$infoIndex] = ['type' => $tokenType === T_FUNCTION ? 'function' : 'class', 'tokenStart' => $MACRO_TOKEN_LOGICAL_START_INDEX(), 'tokenEnd' => null, 'lineStart' => $tokens[$tokenIndex][2], 'lineEnd' => null, 'namespace' => $namespace, 'uses' => $this->getUsesNoScan($namespace), 'name' => null, 'shortName' => null];
                $classBraceCount = 0;
                // start processing with current token
                SCANNER_CLASS_TOP:
                // process the name
                if ($infos[$infoIndex]['shortName'] == '' && (($tokenType === T_CLASS || $tokenType === T_INTERFACE || $tokenType === T_TRAIT) && $infos[$infoIndex]['type'] === 'class' || $tokenType === T_FUNCTION && $infos[$infoIndex]['type'] === 'function')) {
                    $infos[$infoIndex]['shortName'] = is_array($tokens[$tokenIndex + 2]) ? $tokens[$tokenIndex + 2][1] : $tokens[$tokenIndex + 2];
