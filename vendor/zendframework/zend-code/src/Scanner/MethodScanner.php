...
class MethodScanner implements ScannerInterface
{
    ...
    public function getParameter($parameterNameOrInfoIndex)
    {
        ...
        $p = new ParameterScanner(array_slice($this->tokens, $info['tokenStart'], $info['tokenEnd'] - $info['tokenStart']), $this->nameInformation);
    ...
    protected function scan()
    {
        ...
        $MACRO_TOKEN_ADVANCE = function () use(&$tokens, &$tokenIndex, &$token, &$tokenType, &$tokenContent, &$tokenLine) {
            ...
            if (is_string($token)) {
                $tokenType = null;
                $tokenContent = $token;
                $tokenLine += substr_count(isset($lastTokenArray[1]) ? $lastTokenArray[1] : '', "\n");
        ...
        $MACRO_INFO_START = function () use(&$infoIndex, &$infos, &$tokenIndex, &$tokenLine) {
            $infos[$infoIndex] = ['type' => 'parameter', 'tokenStart' => $tokenIndex, 'tokenEnd' => null, 'lineStart' => $tokenLine, 'lineEnd' => $tokenLine, 'name' => null, 'position' => $infoIndex + 1];
        };
